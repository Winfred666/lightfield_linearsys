...
class PointLinearSystem:
    ...
    def __init__(self, A: torch.Tensor, b: torch.Tensor, device: torch.device, threshold_A: float = None, threshold_b: float = None):
        ...
        # Apply masking based on columns (zero out invalid points instead of measurement)
        # - threshold_A controls whether a point column is considered valid.
        ...
        if threshold_A is not None:
            ...
            logger.info(
            "PointLinearSystem: Masked %d/%d columns (%.2f%% kept). Th_A=%s, Th_b=%s",
            total - kept,
            total,
        ...
        # Apply threshold_b only to b values (do not affect column validity)
